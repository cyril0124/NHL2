...
class AsyncBridgeCHI extends RawModule {
    ...
    object AsyncConnect {
        ...
        //      enq_reset  <--/           \--> deq_reset
        // 
        def apply[T <: Data](in: CHIChannelIO[T], name: String = "Unknown", params: AsyncQueueParams = AsyncQueueParams()): CHIChannelIO[T] = {
        ...
        def bitPulseConnect[T <: Data](in: Bool, name: String = "Unknown", depth: Int = 4, sync: Int = 3): Bool = {
    ...
    //
    io.chi_deq.txreq <> AsyncConnect(io.chi_enq.txreq, "enq_txreq_to_deq_txreq")
    io.chi_deq.txdat <> AsyncConnect(io.chi_enq.txdat, "enq_txdat_to_deq_txdat")
    io.chi_deq.txrsp <> AsyncConnect(io.chi_enq.txrsp, "enq_txrsp_to_deq_txrsp")
    ...
    // 
    io.chi_enq.rxdat <> AsyncConnect(io.chi_deq.rxdat, "deq_rxdat_to_enq_rxdat")
    io.chi_enq.rxrsp <> AsyncConnect(io.chi_deq.rxrsp, "deq_rxrsp_to_enq_rxrsp")
    io.chi_enq.rxsnp <> AsyncConnect(io.chi_deq.rxsnp, "deq_rxsnp_to_enq_rxsnp")
