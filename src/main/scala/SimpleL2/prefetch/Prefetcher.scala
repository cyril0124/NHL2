...
/* virtual address */
trait HasPrefetcherHelper extends HasCircularQueuePtrHelper with HasL2Param {
  // filter
...
class PrefetchResp(implicit p: Parameters) extends PrefetchBundle {
  // val id = UInt(tlBundleParams.sourceBits.W)
  val tag = UInt(fullTagBits.W)
  val set = UInt(setBits.W)
  val vaddr = vaddrBitsOpt.map(_ => UInt(vaddrBitsOpt.get.W))
  val pfSource = UInt(MemReqSource.reqSourceBits.W)
...
//   })
  ...
  val pbop = if (hasBOP) Some(
    Module(new PBestOffsetPrefetch()(p.alterPartial({
      case L2ParamKey => p(L2ParamKey).copy(prefetchParams = Seq(SimpleL2.prefetch.BOPParameters(
        virtualTrain = false,
        badScore = 1,
        offsetList = Seq(
